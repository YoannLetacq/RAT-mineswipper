...
pub fn hide_all_mine(all_rect: &mut Vec<Vec<Case>>) {
    let mine = "mine".to_string();
    for row in all_rect {
        for case in row {
            if case.text == mine && case.action == String::new() { // Si la case contient une mine et est révélée
                case.action = "hidden".to_string(); // Cache la mine
            }
        }
    }
}
